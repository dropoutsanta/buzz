...
fn apply_inbound_managed_agent(
    ...
    if let Some(local) = agents.iter_mut().find(|record| record.pubkey == d_tag) {
        ...
        // effective command. Using `record_agent_command` (which resolves
        // override → runtime → persona runtime → default) rather than the
        // context-free `policy_command_for_record` ensures that
        // persona-inherited records (runtime: None after an "inherit" update)
        // follow the live persona runtime instead of the stale agent_command.
